...
class Read():
    ...
    def _initialize_source(self):
        ...
        if not self.cap.isOpened():
            raise ValueError(f"Error: Failed to open {self.source.name} video.")
            ...
            print(f"Successfully opened video {self.name} ({self.width}w x {self.height}h, {self.fps} FPS, {self.frame_count} Frames).")
